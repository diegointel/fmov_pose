...
class Dataset:
    def __init__(self, conf):
        ...
        # NOTE:adapt to the our slightly modified standard
        self.scale_mats_np = []
        self.world_mats_np = []
        for im_name in self.images_lis:
            im_name = os.path.basename(im_name).split(".")[0]
            scale_mat = camera_dict["scale_mat_" + im_name]
            world_mat = camera_dict["world_mat_" + im_name]
            self.scale_mats_np.append(scale_mat.astype(np.float32))
            self.world_mats_np.append(world_mat.astype(np.float32))
    ...
    def gen_random_rays_at(self, img_idx, batch_size, sample_only_on_obj_area=False):
        """
        Generate random rays at world space from one camera.
        """
        if sample_only_on_obj_area:
