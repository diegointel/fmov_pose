...
def get_lineset_object(geometry, color=None):
    x = []
    y = []
    z = []
    ...
    if color is not None:
        line_3d = go.Scatter3d(
            x=x, y=y, z=z, mode="lines", line=dict(width=4, color=color)
        )
    else:
...
def get_graph_objects(
    geometry_list, mesh_show_wireframe=False, point_sample_factor=1, colors=None
):
    graph_objects = []
    for i, geometry in enumerate(geometry_list):
        ...
        if geometry_type == o3d.geometry.Geometry.Type.LineSet:
            if colors is None:
                ...
                graph_objects.append(get_lineset_object(geometry, color=colors[i]))
...
def get_plotly_fig(
    ...
    graph_objects = get_graph_objects(
        geometry_list, mesh_show_wireframe, point_sample_factor, colors
    )
    ...
    if front is not None:
        ...
        plotly_eye = dict(
            x=zoom * 5 * front[0] + plotly_center["x"],
...
def draw_plotly(
    ...
    fig.update_layout(
        scene=dict(
            ...
            # bgcolor='black'  # Set background color to gray
        ),
    )
