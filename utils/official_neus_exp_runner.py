...
from official_neus_models.dataset import Dataset
from official_neus_models.fields import (
    RenderingNetwork,
    SDFNetwork,
    SingleVarianceNetwork,
    NeRF,
)
from official_neus_models.renderer import NeuSRenderer
...
class Runner:
    def __init__(self, conf_path, mode="train", case="CASE_NAME", is_continue=False):
        ...
        self.sample_only_on_obj_area = self.conf.get_bool(
            "train.sample_only_on_obj_area", default=False
        )
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            data = self.dataset.gen_random_rays_at(
                ...
                self.batch_size,
                sample_only_on_obj_area=self.sample_only_on_obj_area
                and self.iter_step > self.warm_up_end,
            )
    ...
    def validate_mesh(self, world_space=False, resolution=64, threshold=0.0):
        ...
        mesh.export(
            os.path.join(
                self.base_exp_dir,
                "meshes",
                "{:0>8d}_{}.ply".format(self.iter_step, resolution),
            )
        )
