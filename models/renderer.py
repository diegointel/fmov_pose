...
class NeuSRenderer:
    ...
    def render(
        ...
        depth_fine = (weights * ret_fine["mid_z_vals"]).sum(dim=-1, keepdim=True)
        return {
    ...
    def extract_color(self, vertices):
        def extract_color(vertices, sdf_network, color_network):
            N = 64
            pts = torch.tensor(vertices).float()
            dirs = torch.zeros_like(pts).float()

            pts = pts.split(N)
            dirs = dirs.split(N)
            out_sampled_color = []
            for pts_batch, dirs_batch in zip(pts, dirs):
                sdf_nn_output = sdf_network(pts_batch)
                feature_vector = sdf_nn_output[:, 1:]
                gradients = sdf_network.gradient(pts_batch).squeeze()
                sampled_color = color_network(
                    pts_batch, gradients, -gradients, feature_vector
                )
        ...
        return extract_color(
            vertices, sdf_network=self.sdf_network, color_network=self.color_network
        )
