# the following code was borrowed from https://github.com/dihuangdh/HHOR/blob/master/NeuS/models/camera.py
...
class Lie:
    ...
    def se3_to_SE3(self, wu, only_rot=False):  # [...,3]
        ...
        V = I + B * wx + C * wx @ wx
        if only_rot:
            return torch.cat([R, torch.zeros_like((V @ u[..., None]).detach())], dim=-1)
...
def procrustes_analysis(X0, X1):  # [N,3]
    ...
    X1c = X1 - t1
    # scale
    s0 = (X0c**2).sum(dim=-1).mean().sqrt() + 1e-8
    s1 = (X1c**2).sum(dim=-1).mean().sqrt() + 1e-8
    X0cs = X0c / s0
    X1cs = X1c / s1
